...
It is the companion command for choosing `ichsm search --columns` and
`ichsm query --columns` values. Start with
[Fields and columns](fields-and-columns.md) for the broader workflow, then use
this page as the command reference.

Without an argument, it lists ENA result types and adds an `ichsm_search`
column showing whether `ichsm search` and `ichsm query` support that type.
Supported types are listed first.
...
`ichsm query --columns`, as shown in [Fields and columns](fields-and-columns.md).
